...
//                Log.d(TAG, "Decoded pitch $value")
            ...
            Protocol.ARDU_GPS_STATUS ->{ //0x5002
                ...
                val gpsAlt: Double = bitExtracted(data.data,7,25)*10.0.pow(
                    bitExtracted(
                        data.data,
                        2,
                        23
...
//                Log.d(TAG, "Decoded flightmode $arduFlightMode")
            ...
            Protocol.ARDU_HOME ->{ //0x5004
                val fr_home_dist = bitExtracted(data.data,10,3) * 10.0.pow(
                    bitExtracted(
                        data.data,
                        2,
                        1
                    )).toDouble()*0.1
                val alt_frome_home = bitExtracted(data.data,10,15)*10.0.pow(
                    bitExtracted(
                        data.data,
                        2,
                        13
            ...
            Protocol.ARDU_VEL_YAW ->{ //0x5005
                val VSpeed = bitExtracted(data.data,7,2) * 10.0.pow(
                    bitExtracted(
                        data.data,
                        1,
                        1
                    )).toDouble()/10f*3.6f //km/h
                val GSpeed = bitExtracted(data.data,7,11) * 10.0.pow(
                    bitExtracted(
                        data.data,
                        1,
                        10
                    )).toDouble()/10f*3.6f //km/h
                val Heading = bitExtracted(data.data,11,17)
