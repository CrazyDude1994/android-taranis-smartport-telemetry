...
public abstract class MediaEncoder implements Runnable {
	...
	/**
	 * We need to send EOF request ASAP with MediaSurfaceEncoder, in drain(),
	 * otherwise it may loop in drain for a long time, recording will not stop
	 *
	 */
	protected boolean mStopRequestSent;
	/**
	...
	protected boolean mIsSurfaceEncoder = false;
...
//		android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
		...
		while (isRunning) {
			...
			if (localRequestStop) {
				...
				//case for not SurfaceEncoder
				if ( !mStopRequestSent )
				{
					mStopRequestSent = true;
...
//********************************************************************************
	...
	protected void signalEndOfInputStream() {
		...
		if (android.os.Build.VERSION.SDK_INT >= 18 && mIsSurfaceEncoder) {
